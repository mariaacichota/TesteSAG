...
type
  ...
  public
    ...
    function ClientExists(ACliente: TCliente): Boolean;
...
function TClienteViewModel.ClientExists(ACliente: TCliente): Boolean;
begin
  Result := False;

  if ACliente.CPF.Trim = EmptyStr then
    Exit;
  ...
  begin
    ACliente.ClienteID := FQuery.FieldByName('ClienteID').AsInteger;
    Result := True;
  end;
end;
